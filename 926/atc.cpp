...
void solve() {
    ...
    while (q--) {
        ...
        int val = tr.make_query(index, index).val;
        cout << "val = " << val << endl;
        tr.make_update(index , index, 0);

        int repeat = val / n;
        tr.make_update(0, n - 1, repeat);

        int rem = val % n;
        if(rem == 0)
            continue;
        int r = min(n - 1 - index, rem);
        rem -= r;
        if(r)
            tr.make_update(index + 1, index + r, 1);
        if(rem)
            tr.make_update(0, rem - 1, 1);
